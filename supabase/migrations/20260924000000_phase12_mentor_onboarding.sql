...
-- Fix Phase 12 public.get_mentor_onboarding_status().
--
-- Two real defects in the original definition:
--   1. `array_agg(mdt) ... ORDER BY mdt.sort_order` is invalid inside an aggregate,
--      which makes the whole function fail at runtime with SQLSTATE 42803
--      ("column mdt.sort_order must appear in the GROUP BY clause or be used in an
--      aggregate function"). The ORDER BY must live inside the aggregate call.
--   2. The "no application yet" branch issued RETURN QUERY but never RETURNed, so
--      execution fell through and dereferenced the NULL v_app.
--
-- The table contract is intentionally NOT changed: `mentor_applications` has no
-- years_of_experience column. Years of experience lives on
...
BEGIN
  -- Most recent application for the current user (draft or otherwise).
  SELECT * INTO v_app
  FROM public.mentor_applications
  ...
  IF NOT FOUND THEN
    -- No application yet: return an empty state with the required document types.
    v_types := ARRAY(
      SELECT mdt
