  ...
  <div className={cn('relative', className)} role="search">
    ...
    <div className="relative flex items-center">
      ...
      <input
        ...
        autoComplete="off"
        className="h-16 w-full rounded-2xl border border-[var(--color-shell-border-strong)] bg-[var(--color-shell-surface-elevated)] pl-16 pr-14 text-[15px] font-medium text-[var(--color-shell-text)] shadow-[var(--seeker-panel-shadow)] outline-none transition-[border-color,box-shadow,background-color] duration-200 placeholder:font-normal placeholder:text-[var(--color-shell-text-subtle)] hover:border-[var(--color-shell-primary)]/40 focus:border-[var(--color-shell-primary)] focus:bg-[var(--color-shell-surface-elevated)] focus:shadow-[0_0_0_4px_var(--color-shell-primary-soft),var(--seeker-panel-shadow)] [&::-webkit-search-cancel-button]:appearance-none"
      />
      {value.length > 0 && (
        <button
          type="button"
